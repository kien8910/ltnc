...
public class Assignment2_test2 {
	...
	public static double [][] normalizationW(double [][] w, double [][] b)
	{
		int row = w.length;
		int col = w[0].length;
		...
		for(int i = 0; i<row; i++)
		{
			for(int j = 0;j<col;j++)
			{
				double tmp = Math.sqrt(w[i][j]*w[i][j]+b[i][1]*b[i][1]);
				ret[i][j] = w[i][j]/tmp;
			}
	...
	public static void main(String[] args) {
		...
		for(int p = 0; p<5;p++)
		{
			...
			for(int i = 0; i<L-1;i++)
			{
				...
				double [][]_tmp = new double [numNode[i+1]][numNode[i]];
				deltaW.add(_tmp);
				
				_tmp = new double [numNode[i+1]][1];
				delta_b.add(_tmp);
			}
			...
			for(int i = 0;i<x.size();i++)
			{
				...
				printMatrix(a.get(L-1));
				System.out.println();
				double tmp2 [][] = {{target.get(i)}};
				g.set(L-1, subtractMatrix(a.get(L-1),tmp2)); //modify
				//d.set(L-1, calculate_d(a.get(L-1)));
				
				for(int l=L-2;l>=0;l--)
				{
					dW.set(l,mulMatrix(g.get(l+1),transpose(a.get(l))));
					db.set(l, g.get(l+1));
					g.set(l, mulMatrix(transpose(W.get(l)),g.get(l+1)));
				//	d.set(l,calculate_d(a.get(l)));
				//	printMatrix(dW.get(l));
				//	System.out.println();
				}
				
				for (int l = 0;l<L-1;l++)
				{
					//double [][] params1 = mulMatrixWithNumber(0.9,dW.get(l));
					//W.set(l, subtractMatrix(W.get(l),params1));
					//params1 = mulMatrixWithNumber(0.9,db.get(l));
				...
				//printMatrix(deltaW.get(l));
			
			for (int l = 0;l<L-1;l++)
			{
				
				double [][] params1 = mulMatrixWithNumber(1.0/x.size(),deltaW.get(l));
				//double [][] params2 = mulMatrixWithNumber(0.9,W.get(l));
				//deltaW.set(l, addMatrix(params1,params2));
				
				deltaW.set(l,params1);
				//printMatrix(deltaW.get(l));
			//	System.out.println();
				delta_b.set(l, mulMatrixWithNumber(1.0/x.size(),db.get(l)));
				params1 = mulMatrixWithNumber(0.1,deltaW.get(l));
				W.set(l, subtractMatrix(W.get(l),params1));
				params1 = mulMatrixWithNumber(0.1,delta_b.get(l));
				b.set(l, subtractMatrix(b.get(l),params1));
